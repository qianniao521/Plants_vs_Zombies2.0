...
class Zombie5:
    def __init__(self, r):
        ...
        self.im1 = load_image("game_picture/zombies/zombie5/1")
        self.im2 = load_image("game_picture/zombies/zombie5/2")
        self.im3 = load_image("game_picture/zombies/zombie5/3")
    ...
    def action(self, model):
        ...
        if self.status == 1:
            self.pos0 = pos_conversion_z(self.pos, self.size[self.status], -50)  # 图片显示位置
        else:
            self.pos0 = pos_conversion_z(self.pos, self.size[self.status], self.offset) # 图片显示位置
        if self.ice_time >= 0:
            if self.ice_time <= 0:
