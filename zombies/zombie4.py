...
class Zombie4:
    def __init__(self, r):
        ...
        self.im1 = load_image("game_picture/zombies/zombie4/1")
        self.im2 = load_image("game_picture/zombies/zombie4/2")
        self.im3 = load_image("game_picture/zombies/zombie4/3")
        self.im4 = load_image("game_picture/zombies/zombie4/4")
        self.im5 = load_image("game_picture/zombies/zombie4/5")
        self.im6 = load_image("game_picture/zombies/zombie4/6")
        self.im = [self.im0, self.im1, self.im2, self.im3, self.im4, self.im5, self.im6]
        self.image = self.im[self.status][self.actions]
        self.size = []  # 不同状态下的大小
        for i in self.im:
            self.size.append(i[0].get_size())
        self.w = self.size[2][0] // 3 # 图片宽度，血条长度
        ...
        self.attack_distance = lawn_cube_size[0] // 2 - 1 # 攻击距离
    ...
    def action(self, model):
        if self.HP <= 0 and self.status > 1:
            self.status = 1
            self.actions = 0
        elif self.status == 2:#冲刺
            self.move()
            self.move()#冲刺状态下速度是正常的两倍，故移动两次
            self.attack_range = [self.pos[0] - self.attack_distance, self.pos[0] + self.attack_distance]  # 攻击范围
            for i in range(len(model.plants)):
                if self.r == model.plants[i].r:
                    if self.attack_range[0] < model.plants[i].pos[0] < self.attack_range[1]:  # 跳跃触发范围
                        self.status = 3
                        self.actions = 0
                        self.time = 0
                        break
        elif self.status == 3:#撑杆跳跃
            self.time += 1
            if self.time == (len(self.im[self.status]) - 1) * self.fs:
                self.status = 4
                self.actions = 0
                self.pos[0] -= lawn_cube_size[0]
                self.time = 0
        elif self.status == 4:#跳跃落地
            self.time += 1
            if self.time == (len(self.im[self.status]) - 1) * self.fs:
                self.status = 5
        elif self.status == 5:#行走
            self.move()
            self.attack_range = [self.pos[0] - self.attack_distance, self.pos[0] + self.attack_distance]  # 攻击范围
            for i in range(len(model.plants)):
                if self.r == model.plants[i].r:
