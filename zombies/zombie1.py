...
class Zombie1:
    def __init__(self, r):
        ...
        self.im2 = load_image("game_picture/zombies/zombie1/2")
        self.im3 = load_image("game_picture/zombies/zombie1/3")
...
"""elif 3 > self.status > 0:
status = 1
for i in range(len(plants)):
    if self.r == plants[i].r:
        if self.attack_range[0] < plants[i].pos[0] < self.attack_range[1]:  # 攻击状态
            status = 2
            if status != self.status:
                self.status = status
                self.actions = 0
                self.frame = 0
            self.actions = (self.actions + self.frame) % 21
            self.frame = (self.frame + 1) % 2
            self.attack(plants, i)
            self.image = self.im[self.status][self.actions]
            return
if status != self.status:  # 移动状态
    self.status = status
    self.actions = 0
    self.frame = 0
self.move()
